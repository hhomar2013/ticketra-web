<div>
    @push('styles')
    <style>
        .stat-card { border-radius: 16px; border: none; transition: transform .2s, box-shadow .2s; overflow: hidden; }
        .stat-card:hover { transform: translateY(-4px); box-shadow: 0 12px 28px rgba(0,0,0,.1) !important; }
        .stat-card .icon-box { width: 48px; height: 48px; border-radius: 13px; display: flex; align-items: center; justify-content: center; font-size: 20px; }
        .stat-card .stripe { height: 4px; width: 100%; }

        .filter-btn { border: 1.5px solid #e5e7eb; border-radius: 10px; padding: 7px 14px; font-size: 12px; font-weight: 600; color: #6b7280; background: #fff; cursor: pointer; transition: all .2s; display: inline-flex; align-items: center; gap: 6px; }
        .filter-btn:hover  { border-color: #0d6efd; color: #0d6efd; background: rgba(13,110,253,.04); }
        .filter-btn.active { background: linear-gradient(135deg,#0d6efd,#0047c4); border-color: transparent; color: #fff; box-shadow: 0 4px 12px rgba(13,110,253,.25); }

        .search-wrap { position: relative; }
        .search-wrap .s-ico { position: absolute; left: 13px; top: 50%; transform: translateY(-50%); color: #9ca3af; font-size: 13px; pointer-events: none; }
        .search-input { width: 100%; border: 1.5px solid #e5e7eb; border-radius: 12px; padding: 10px 14px 10px 38px; font-size: 14px; color: #111827; background: #fafafa; transition: all .2s; outline: none; }
        .search-input:focus { border-color: #0d6efd; background: #fff; box-shadow: 0 0 0 4px rgba(13,110,253,.08); }

        .table-wrap { border: 1px solid #e5e7eb; border-radius: 12px; overflow: visible; }
        .action-drop .dropdown-menu { border: 1px solid #e5e7eb; border-radius: 14px; box-shadow: 0 8px 24px rgba(0,0,0,.1); padding: 6px; min-width: 170px; z-index: 9999; }
        .action-drop .dropdown-item { font-size: 13px; padding: 8px 14px; border-radius: 8px; display: flex; align-items: center; gap: 8px; }
        .action-drop .dropdown-item:hover { background: #f3f4f6; }
    </style>
    @endpush
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-2">
                                <div style="width:32px;height:32px;border-radius:9px;background:rgba(13,110,253,.1);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#0d6efd;">
                        ...
                        <td>
                            <a href="{{ route('invoices.batches', $invoice->id) }}" wire:navigate
                        ...
                        <td>
                            @if($invoice->status === 'paid')
                                <span style="background:#d1e7dd;color:#0f5132;border-radius:20px;padding:4px 12px;font-size:11px;font-weight:700;">
                                    <span style="width:6px;height:6px;border-radius:50%;background:#198754;display:inline-block;margin-right:4px;"></span>
                                    {{ __('Paid') }}
                                </span>
                            @else
                                <span style="background:#fff3cd;color:#664d03;border-radius:20px;padding:4px 12px;font-size:11px;font-weight:700;">
                                    <span style="width:6px;height:6px;border-radius:50%;background:#ffc107;display:inline-block;margin-right:4px;"></span>
                                    {{ __('Pending') }}
                                </span>
                            @endif
                        </td>
                        <td class="pe-4 text-end action-drop">
                            <div class="dropdown {{ $loop->remaining < 2 ? 'dropup' : '' }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    <li>
                                        <a class="dropdown-item" href="{{ route('invoices.edit', $invoice->id) }}" wire:navigate>
                                            <i class="fas fa-edit text-primary"></i> {{ __('Edit') }}
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="{{ route('invoices.batches', $invoice->id) }}" wire:navigate>
                                    ...
                                    @if($invoice->status === 'pending')
                                        <li>
                                            <button class="dropdown-item"
                                                ...
                                                <i class="fa fa-check-circle text-success"></i> {{ __('Mark as Paid') }}
                                            </button>
                                        </li>
                                    @else
                                        <li>
                                            <button class="dropdown-item"
                                                ...
                                                <i class="fa fa-clock text-warning"></i> {{ __('Mark as Pending') }}
                                            </button>
                                        </li>
                                    @endif
                                    <li><hr class="dropdown-divider my-1"></li>
