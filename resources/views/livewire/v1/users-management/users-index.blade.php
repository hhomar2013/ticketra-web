<div>
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('user-management.roles') }}" wire:navigate
                class="btn btn-outline-secondary btn-sm rounded-pill px-3 fw-bold" style="font-size: 13px;">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
        <div class="d-flex flex-wrap gap-2">
            ...
            @foreach ($roles as $role)
                <button wire:click="$set('roleFilter', '{{ $role->name }}')"
                    class="filter-btn {{ $roleFilter === $role->name ? 'active' : '' }}">
                    {{ ucfirst($role->name) }}
                </button>
            @endforeach
        </div>
            ...
            <table class="table table-hover align-middle mb-0">
                <thead style="background: #f9fafb;">
                    <tr>
                        ...
                        <th class="py-3 text-muted small">{{ __('Department') }}</th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        @if ($user->id === auth()->id())
                                            <span
                                                style="font-size: 10px; color: #0d6efd; font-weight: 700;">{{ __('You') }}</span>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                @foreach ($user->roles as $role)
                                    <span
                                        style="background:
                                        rgba(13,110,253,.08); color: #0d6efd; border-radius: 20px; padding: 3px 10px;
                            ...
                            <td>
                                <span
                                    style="background: rgba(13,110,253,.08); color: #ff9602;
                                    border-radius: 20px; padding: 3px 10px; font-size: 11px; font-weight: 700;">
                                    {{ $user->category->name ?? '__' }}
                                </span>
                            </td>
                            <td>
                                @if ($user->status ?? true)
                                    <span
                                        style="background:#d1e7dd;color:#0f5132;border-radius:20px;padding:3px 10px;font-size:11px;font-weight:700;">
                                        <span
                                ...
                                @else
                                    <span
                                        style="background:#e2e3e5;color:#41464b;border-radius:20px;padding:3px 10px;font-size:11px;font-weight:700;">
                                        {{ __('Inactive') }}
                                    </span>
                                @endif
                            </td>
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('user-management.edit', $user->id) }}" wire:navigate>
                                                <i class="fas fa-edit text-primary"></i> {{ __('Edit') }}
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('user-management.roles') }}?user={{ $user->id }}"
                                                wire:navigate>
                                        ...
                                        @if ($user->id !== auth()->id())
                                            ...
                                            <li>
                                                <button class="dropdown-item"
                                                    onclick="confirmDelete({{ $user->id }}, 'delete-user')">
